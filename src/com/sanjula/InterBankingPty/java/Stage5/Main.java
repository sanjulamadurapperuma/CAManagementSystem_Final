...
public class Main {
    protected static final String SEPARATOR_STRING = "***********************" +
            "***********************************************";
    private static File customerDetails = new File("CustomerDetailsFile.txt");//Customer objects are stored here
    public static List<Customer> customerList = new ArrayList<>();//Stores all customer objects
    public static List<BankAccount> bankAccountList = new ArrayList<>();//Stores all the bank accounts of a customer
    ...
    public static void main(String[] args){
        ...
        do{//If selected option = 0, exit the program
            ...
            if(selectedOption == 1){//Creates a new bank account
                ...
                if (customer != null) {
                    transfer(customer);//Calling the transfer method passing in the customer object
                } else{
    ...
    private static int getSelectedOption(){//validates customer's menu
        Scanner sc = new Scanner(System.in);
        while (!sc.hasNextInt()){
    ...
    public static int getAccountBalance(Scanner scanner, String message){
        int accountBalance;
        do{
            System.out.print(message);
            while(!scanner.hasNextInt()){
    ...
    public static double getInterestRate(Scanner scanner, String message){
        double interestRate;
        do {
            System.out.print(message);
            ...
            if(!(interestRate >= 0.01 || interestRate <= 15.0)){
                System.err.println("Invalid interest rate!!! It has to be a positive number.");
            }
